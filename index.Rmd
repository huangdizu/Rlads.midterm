...
artists <- list.files(path = "C:/Users/huang/Rlads/midterm/lyrics", full.names = F)

songs <- list.files(path = "./lyrics/Lady Gaga", full.names = T)
...
#這題看不懂，直接上助教解
# Answer 1
lyrics <- vector("list", length(artists))
names(lyrics) <- artists

for (name in artists) {
   fp <- paste0("./lyrics/", name)
   songs <- list.files(path = fp, full.names = T)
   lyrics[[name]] <- songs
}

# Answer 2
lyrics <- lapply(artists, function(x) {
   fp <- paste0("./lyrics/", x)
   list.files(path = fp, full.names = T)
})

# Answer 3 (Answer1精簡版)
lyrics <- list()
for (x in artists) {
  lyrics[[x]] <- list.files(file.path("./lyrics", x), full.names = TRUE)
}
...
library(dplyr)
anime <- anime %>% 
  select(Name, Genders, Type, Source, Score, Popularity) %>%
...
ggplot(mpg, mapping = aes(x = cty, y = hwy)) +
  geom_point() +
  geom_smooth(method = "lm") + #  geom_smooth(method="lm", formula = y ~ x)
  facet_wrap(vars(year)) + #facet_wrap(~factor(year))
...
#Answer 1
compare_plot <- function(df, countries, start, end, point = FALSE) {
  data <- df %>%
  ...
  if(point){
    ggplot(data, mapping = aes(x = Date, y = Confirmed, color = Country)) +
      geom_line() +
      geom_point()
  } else {
    ggplot(data, mapping = aes(x = Date, y = Confirmed, color = Country)) +
...
compare_plot <- function(df, countries, start, end, point = FALSE) {
  data <- df %>%
    filter(Country %in% countries, Date >= as.Date(start) & Date <= as.Date(end))
  ggplot(data = data, aes(x = Date, y = Confirmed, color = Country)) +
    geom_line() +
    if (point) geom_point()
}
```
...
# Write your code here
kid <- rnorm(N, mean = ifelse(gender==1, 1.5*dad + mom, 1.5*mom + dad))

df <- data.frame(dad, mom, gender, kid)

#助教解
# Simulate kid's skill (moderated by gender)
kid <- vector("double", length = N)
for (i in seq_along(gender)) {
  # Differential effects based on kid's gender
  if (gender[i] == 1) 
    m <- 1.5*dad[i] + 1.0*mom[i]
  else 
    m <- 1.0*dad[i] + 1.5*mom[i]
  kid[i] <- rnorm(n = 1, mean = m)
}
